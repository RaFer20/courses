...
# Solution
In the original code `+` is converting the arrays into strings before concatenating them, so `dishName` and `dish` are string, there is no `join()` metho for strings.

If we use concat instead the issue is resolved.
```js
// Picks n random elements from an array,
// and returns a new array with those elements.
function random(array, n) {
