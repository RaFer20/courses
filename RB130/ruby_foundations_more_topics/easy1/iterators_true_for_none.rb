=begin
In the previous two exercises, you developed methods called any? and all? that are similar to the standard Enumerable
methods with the same names. In this exercise, you will develop another of the methods in this family, none?.
...
Otherwise, #none? returns true. Note in particular that #none? will stop searching the collection the first time the block returns true.
...
# Alternate solution using our `any?` method
def any?(array)
...
def none?(collection, &block) # the block needs to be passed to any? explicitly
