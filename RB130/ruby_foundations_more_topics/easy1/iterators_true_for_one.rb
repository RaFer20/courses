...
If the block returns a truthy value for exactly one element, then #one? returns true. Otherwise, #one? returns false.
...
def one?(arr)
  counter = 0
  arr.each{|value| counter += 1 if yield(value); return false if counter == 2}
  counter == 1
end
