...
such as Arrays and Hashes. These methods make writing certain code a bit easier, at least when compared to a while or for loop.
...
you may define what values are iterated upon. We'll want to create an Enumerator that can iterate over an infinite list of factorials.
...
The factorial values (0!, 1!, 2!, etc.) follow a simple pattern.
First, 0! and 1! are both defined to be 1. All subsequent factorials are the result of multiplying all the
integers from 1 up to the n value together. Thus, 5! is 5 * 4 * 3 * 2 * 1, or 120.
