...
=begin
The error was caused because we were calling  `display_data` on the `books` array in library
and not on the individual book objects.

We can call it on each book instead or create a method on library to call it on the books array.
=end