...
=end
class Student
  def initialize(name=nil, year="hidden")
    @name = name
    @year = year
  end
end
...
This calls the superclass method of the same name as the calling method, but here no arguments are passed to the superclass method at all.

Can you think of a way to use super() in another Student related class?
=end
class Student
  def initialize(name="hidden", year="hidden")
    @name = name
    @year = year
  end
end
...
sean = InternationalStudent.new("secret")